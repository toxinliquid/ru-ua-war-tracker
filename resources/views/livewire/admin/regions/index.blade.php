<div class="space-y-6">
    ...
    <div class="flex flex-col justify-between gap-3 md:flex-row md:items-center">
        <div>
            <flux:heading size="lg">{{ __('Region Control Tracker') }}</flux:heading>
            <flux:text class="mt-1">
                {{ __('Manage oblast-level control estimates. Enter either side and we will fill in the remainder.') }}
            </flux:text>
        </div>
        <flux:button wire:click="create" icon="plus">{{ __('Add New Region') }}</flux:button>
    </div>
    ...
    <flux:card class="space-y-6">
        <form wire:submit.prevent="save" class="space-y-4">
            ...
            <flux:input
                ...
                placeholder="e.g., Donetsk Oblast"
            />
            ...
            <div class="flex justify-end gap-2">
                ...
                <flux:button type="submit" icon="check">
                    {{ $editingId ? __('Save changes') : __('Add region') }}
                </flux:button>
            </div>
        </form>
    </flux:card>
    ...
    <flux:card class="space-y-4">
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-zinc-200 text-sm dark:divide-zinc-700">
                ...
                <tr>
                    <th class="px-4 py-3 text-left">{{ __('Region') }}</th>
                ...
                @forelse ($regions as $region)
                    <tr>
                        ...
                        <td class="px-4 py-3 align-top text-rose-500 dark:text-rose-300">
                            {{ number_format($region->russian_control_percent, 2) }}%
                        </td>
                        <td class="px-4 py-3 align-top text-emerald-500 dark:text-emerald-300">
                            {{ number_format($region->ukrainian_control_percent, 2) }}%
                        </td>
                        <td class="px-4 py-3 align-top text-zinc-500 dark:text-zinc-300">
                            {{ number_format($region->neutral_control_percent, 2) }}%
                        </td>
                        <td class="px-4 py-3 align-top text-zinc-500 dark:text-zinc-300">
                            {{ optional($region->updated_at)->diffForHumans() }}
                        </td>
                        <td class="px-4 py-3 align-top text-right">
                            <div class="flex justify-end gap-2">
                                ...
                                <flux:button
                                    ...
                                    icon="trash"
                                    x-data
                                    x-on:click.prevent="if (confirm('{{ __('Delete :region?', ['region' => $region->name]) }}')) $wire.delete({{ $region->id }});"
                                >
                                    {{ __('Delete') }}
                                </flux:button>
                            </div>
                ...
                @empty
                    <tr>
                        <td colspan="6" class="px-4 py-10 text-center text-zinc-500 dark:text-zinc-400">
                            {{ __('No regions tracked yet. Add your first region above.') }}
                        </td>
                    </tr>
                @endforelse
                </tbody>
