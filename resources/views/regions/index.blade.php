...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}" class="dark">
    <head>
        @include('partials.head', ['title' => __('Regional Control Overview') . ' | ' . config('app.name')])
    </head>
    <body class="min-h-screen bg-gradient-to-br from-slate-950 via-zinc-900 to-black text-zinc-100">
        <div class="px-6 py-10 sm:px-10 lg:px-16">
            <div class="mx-auto flex max-w-5xl flex-col gap-10">
                <header class="space-y-4 text-center">
                    <h1 class="text-4xl font-semibold tracking-tight sm:text-5xl">
                        {{ __('Regional Control Overview') }}
                    </h1>
                    <p class="text-sm text-zinc-300 sm:text-base">
                        {{ __('Current overview of oblast-level control between Russian and Ukrainian forces.') }}
                    </p>
                ...
                <section class="space-y-6">
                    @forelse ($regions as $region)
                        @php
                            $ru = max(0, min(100, $region->russian_control_percent));
                            $ua = max(0, min(100, $region->ukrainian_control_percent));
                            ...
                            <div class="flex flex-col gap-3 sm:flex-row sm:items-baseline sm:justify-between">
                                <div>
                                    ...
                                    <p class="text-xs uppercase tracking-[0.28em] text-zinc-400">
                                        {{ __('Updated :time', ['time' => optional($region->updated_at)->diffForHumans() ?? __('recently')]) }}
                                    </p>
                                </div>
                    ...
                    @empty
                        <div class="rounded-2xl border border-dashed border-white/10 p-10 text-center text-zinc-400">
                            {{ __('No regional control data available yet.') }}
                        </div>
                    @endforelse
                </section>
